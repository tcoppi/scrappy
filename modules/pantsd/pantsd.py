...
class pantsd(Module):
    ...
    def pants(self, server, event, bot):
        ...
        if acct == None:
            c.privmsg(event.target, "%s, sorry, but you don't have an account." % event.source.nick)
            return
        if len(event.arg) > 0:
            status = event.arg[0].lower()
            if status not in ("on","off"):
                c.privmsg(event.target, "%s, your pants can either be \x02on\x02 or \x02off\x02." % event.source.nick)
                return

            if status == "on":
                c.privmsg(event.target, "%s, I see you put your pants on." % event.source.nick)
            else:
                c.privmsg(event.target, "%s, I see you took your pants off." % event.source.nick)
            ...
            try:
                ...
                if acct_pants.status:
                    c.privmsg(event.target, "%s, your pants are on!" % event.source.nick)
                else:
                    c.privmsg(event.target, "%s, your pants are off!" % event.source.nick)
            except Pants.DoesNotExist:
                c.privmsg(event.target, "%s, I don't know whether your pants are on or off!" % event.source.nick)
